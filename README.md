...
- **Панель процедур/функций** (outline) справа с фильтром, сортировкой, сворачиванием всех блоков и изменяемым размером
- **Группировка по областям** (#Область / #Region) в панели процедур
- **Режим редактирования** с сохранением (Ctrl+S), форматированием BSL (Alt+Shift+F) и комментированием строк (Ctrl+/)
...
- Полный набор функций: панель процедур, фильтр, сортировка, форматирование, редактирование, сохранение
...
### Язык запросов 1С (`.sdbl` / `.query`)

Ключевые слова запроса, параметры `&Имя` и функции вроде `СУММА` подсвечиваются отдельно от модуля BSL.

**Lister F3**

![Запрос во вьюере](screens/viewer-query.png)

### Markdown (`.md`) — исходник слева, превью справа

Активная строка и выделенный фрагмент подсвечиваются в обеих панелях, чтобы было видно соответствие при разном кегле.

**BSLEdit**

![Markdown в BSLEdit](screens/bsledit-md.png)

**Lister F3** (тёмная тема)
